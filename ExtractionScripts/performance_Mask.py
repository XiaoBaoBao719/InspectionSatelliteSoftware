""" Returns FP_RGB_record from [INPUT]

Determines detections of interest and extracts their data
...
def Performance(det_dict, viaPolygon, iou_threshold, MAX_TP, MAX_FP, MAX_FP_RGB, CONFIDENCE_SIG_FIGS):
  """ Determines detections of interest and extracts 
      their data

      Input: 
      --> External: viaPolygon.json [type file], det_dict = inference_Mask.Inference_Mask [type dict]
  ...
  # add TP_record and FP_record bbox and confidences to outputData.json
...
# # external inputs
# from inference_Mask import Inference_Mask
# import cv2

# im = cv2.imread("capImg_multi.jpg")                  # located in same directory as inference_mask.py
# inference_threshold = 0.02
# det_dict = Inference_Mask(im,inference_threshold)    # dictionary of detections from inference_Mask.py
# viaPolygon = "viaPolygon.json"
...
# FP_RGB_record = Performance(det_dict, viaPolygon, iou_threshold, MAX_TP, MAX_FP, MAX_FP_RGB, CONFIDENCE_SIG_FIGS)